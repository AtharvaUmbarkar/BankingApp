...
public interface TransactionRepo extends JpaRepository<Transaction, Integer>{
	...
	@Query("SELECT t, t.senderAccount.accountNumber as senderAccountNumber, t.receiverAccount.accountNumber as receiverAccountNumber from Transaction t WHERE (t.senderAccount.accountNumber =?1 OR t.receiverAccount.accountNumber = ?1) AND (t.txnDate >= ?2 AND t.txnDate <= ?3) ORDER BY t.txnDate DESC LIMIT 10")
